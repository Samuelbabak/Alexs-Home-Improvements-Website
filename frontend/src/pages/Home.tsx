...
const Home: React.FC = () => {
  const exampleProjects = PROJECTS.slice(0, 6);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {exampleProjects.map((project) => (
              <ProjectCard 
                ...
                image={project.coverImage ?? project.images.filter((imagename) => imagename.startsWith('after'))[0]} 
              />
            ))}
          </div>
        </div>
...
const ProjectCard: React.FC<{id: string, title: string; location: string | undefined; description: string | undefined; image: string}> = ({ id, title, location, description, image }) => (
  <div className="bg-white max-w-sm rounded-xl overflow-hidden shadow-lg hover:shadow-on-hover transition-all duration-300 group">
    <a href={`/gallery/${id}`}>
      ...
      <div className="p-6">
        <div className="flex justify-between items-center mb-2">
          ...
          <span className="text-xs font-medium text-gray-500 bg-gray-100 px-2 py-1 rounded uppercase">{location ? location : 'Asheville, NC'}</span>
        </div>
